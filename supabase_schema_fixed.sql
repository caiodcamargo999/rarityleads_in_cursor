-- =====================================================
-- RARITY LEADS - SUPABASE DATABASE SCHEMA (FIXED)
...
-- Function to calculate lead score (FIXED VERSION)
CREATE OR REPLACE FUNCTION calculate_lead_score(input_lead_id UUID)
RETURNS INTEGER AS $$
DECLARE
    score INTEGER := 0;
    lead_data RECORD;
...
BEGIN
    ...
    IF lead_data.company_id IS NOT NULL THEN
        SELECT * INTO company_data
        FROM public.companies
        WHERE id = lead_data.company_id;
    END IF;

    -- Base score factors
    IF lead_data.email IS NOT NULL THEN score := score + 10; END IF;
    IF lead_data.phone IS NOT NULL THEN score := score + 10; END IF;
    IF lead_data.linkedin_url IS NOT NULL THEN score := score + 15; END IF;

    -- Job title scoring
    CASE lead_data.seniority_level
    ...
    IF company_data.id IS NOT NULL THEN
        CASE company_data.size_category
...
BEGIN
    ...
    -- Log the score change if it's different
    IF old_score != new_score THEN
        INSERT INTO public.lead_scoring_history (lead_id, previous_score, new_score, scoring_factors)
        VALUES (NEW.id, old_score, new_score, jsonb_build_object('auto_calculated', true, 'timestamp', NOW()));
    END IF;
...
BEGIN
    ...
    RAISE NOTICE '📊 Tables: 15 core tables + views and functions';
    RAISE NOTICE '🔐 Security: RLS enabled on all tables';
    RAISE NOTICE '⚡ Performance: Indexes and triggers active';
    RAISE NOTICE '🎯 Features: Multi-WhatsApp, Lead scoring, Analytics';
